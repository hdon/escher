          ...
          static if (1)
          {
            ...
            switch (words[11])
            {
              case "player":
                ...
                playerSpawner = EntityPlayer.spawner(spaceID, translation);
...
class Camera
{
  ...
  void key(int keysym, bool down)
  {
    ...
    {
      case 's':
        playerEntity.keyForward = down;
        break;

      case 'w':
        playerEntity.keyBackward = down;
        break;

      case 'a':
        playerEntity.keyLeft = down;
        break;
      
      case 'd':
        playerEntity.keyRight = down;
        break;

      case ' ':
        playerEntity.keyUp = down;
        break;

      case 'c':
        playerEntity.keyDown = down;
        break;
      ...
      case 'l':
        if (down)
          vertexer.lightPos = vec3f(playerEntity.pos.xyz);
        break;
  ...
  Entity aimFind(mat4 mvmat, mat4 pmat)
  {
    ...
    foreach (iEnt, ent; world.entities[playerEntity.spaceID])
    {
  ...
  /* TODO this should no longer be needed -- will be handled by entity module? */
  void collideWithEntities()
  {
    foreach (iEntity, entity; world.entities[playerEntity.spaceID])
    {
      ...
      if ((entity.pos - playerEntity.pos).magnitude_squared <= sq(entity.getHitSphereRadius() + 0.35))
      {
        //entity.dead = true;
  ...
  void draw()
  {
    ...
    world.drawSpace(playerEntity.spaceID, mvmat, maxPortalDepth, 0);
